...
![ModelData](SQL.png)
## Оценка удельного объема информации, хранимой в модели
Пусть у нас есть N1 пользователей каждый работает в N2 репозиториях (все во всех) Имеется по N4 коммитов, N3 PR и N5 Issue
Тогда по сравнению с нереаляционной мы задействуем на N2*(N1+N3+N4+N5)*(8+8)+N4*(8+8) bytes на свяжуюшие таблицы, но сэкономим на Полях перечисления.
